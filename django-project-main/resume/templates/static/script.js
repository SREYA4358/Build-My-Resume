...
if (document.body.classList.contains("builder-page")) {
  ...
  function bindInput(
    ...
    // Remove 'r' prefix from output element ID to get section ID
    // e.g., 'rSummary' -> 'Summary' -> 'summarySection'
    const baseId = outputElement.id.startsWith('r') ? outputElement.id.substring(1) : outputElement.id;
    const sectionId = baseId.charAt(0).toLowerCase() + baseId.slice(1) + "Section";
